"""Binance USDT-M Futures WS — 雙連線 pattern（markPrice array + bookTicker）

- markPrice/index/funding: wss://fstream.binance.com/market/stream?streams=!markPrice@arr@1s
  全市場 array stream，filter 出我們關心的 symbol
- bookTicker: wss://fstream.binance.com/ws + SUBSCRIBE 個別 symbol
"""
...
WS_BOOK_URL = "wss://fstream.binance.com/ws"


class BinanceFuturesSource:
    def __init__(self, watch_symbols: set[str]):
        self.watch_symbols = {s.upper() for s in watch_symbols}
        self._mark: dict[str, dict] = {}  # symbol upper -> {mark, index, funding, ts}
        self._book: dict[str, dict] = {}  # symbol upper -> {bid, ask, last, ts}
    ...
    async def wait_ready(self, timeout: float = 10.0):
        try:
            ...
            logger.warning("[binance-ws] wait_ready timeout")
    ...
    async def _run_book(self):
        ...
        while True:
            try:
                ...
                logger.info(f"[binance-ws] bookTicker 連線，訂閱 {len(streams)} streams")
                ...
                logger.warning(f"[binance-ws] bookTicker 中斷 {backoff}s 後重連: {e}")
                await asyncio.sleep(backoff)
                backoff = min(backoff * 2, 30)
