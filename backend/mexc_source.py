"""MEXC futures WS — sub.ticker（跟 Ourbit 同格式、Ourbit 是 MEXC 白標）

wss://contract.mexc.com/edge，無需 API key。
"""
...
class MexcFuturesSource:
    ...
    async def wait_ready(self, timeout: float = 10.0):
        try:
            ...
            logger.warning("[mexc-ws] wait_ready timeout")
    ...
    async def _run(self):
        ...
        while True:
            try:
                logger.info(f"[mexc-ws] 連線 {WS_URL}，預計訂閱 {len(self.watch_symbols)} symbols")
                async with websockets.connect(WS_URL, ping_interval=None) as ws:
                    for sym in sorted(self.watch_symbols):
                        await ws.send(json.dumps({"method": "sub.ticker", "param": {"symbol": sym}}))
                        await asyncio.sleep(0.05)
                    logger.info(f"[mexc-ws] 已訂閱 {len(self.watch_symbols)} symbols")
                ...
                logger.warning(f"[mexc-ws] 中斷 {backoff}s 後重連: {e}")
